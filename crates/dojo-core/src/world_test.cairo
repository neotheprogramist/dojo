...
#[test]
#[available_gas(6000000)]
fn test_merger_program_hash_event_emit() {
    let world = deploy_world();
    drop_all_events(world.contract_address);
    let config = IConfigDispatcher { contract_address: world.contract_address };

    config.set_merger_program_hash(program_hash: 98758347158781475198374598718743);

    assert_eq!(
        starknet::testing::pop_log(world.contract_address),
        Option::Some(MergerProgramHashUpdate { program_hash: 98758347158781475198374598718743 })
    );
}
