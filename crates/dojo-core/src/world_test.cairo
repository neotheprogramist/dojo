...
trait Ibar<TContractState> {
    ...
    fn set_fizz(self: @TContractState, a: felt252, b: felt252);
}
    ...
    impl IbarImpl of super::Ibar<ContractState> {
        ...
        fn delete_foo(self: @ContractState) {
            ...
            self
                .world
                .read()
                .delete_entity('Foo', array![get_caller_address().into()].span(), array!['caller'].span(), layout.span()); // this should be a macro
        }
...
fn test_entities() {
    ...
    let mut where = MemberClause { model: 'Fizz', member: 'a', value: 1337, };

    // Get all with a == 1337
    let values = world.entities(Clause::Member(where), 1, layout);
    assert(values.len() == 2, 'Not all keys found for 1337!');
    assert(*(*values.at(0)).at(0) == 0x1337, 'Caller at 0 not valid');
    ...
    where.value = 420;
    let values = world.entities(Clause::Member(where), 1, layout);
    assert(values.len() == 1, 'Not all keys found for 420!');
